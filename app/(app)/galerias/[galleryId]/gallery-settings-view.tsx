...
import { Calendar, Download, Heart, Loader2, Lock, Settings2 } from "lucide-react";
...
export type SettingsSection = "geral" | "privacidade" | "download" | "favoritos";
...
const SECTION_META: Record<SettingsSection, { title: string; hint: string }> = {
  geral: { title: "Em geral", hint: "Link público e modo da galeria." },
  privacidade: { title: "Privacidade", hint: "Senha e expiração de acesso." },
  download: { title: "Download", hint: "Permita que o cliente baixe as fotos." },
  favoritos: { title: "Favoritos", hint: "Permita o cliente favoritar e enviar a seleção." },
};
...
export function GallerySettingsView({ gallery, section, onUpdate }: Props) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-ds-hairline px-5 py-4 md:px-8">
        <h2 className="text-lg font-semibold text-ds-ink">{meta.title}</h2>
        <p className="mt-0.5 text-sm text-ds-muted">{meta.hint}</p>
      </div>
        ...
        {section === "privacidade" && (
          <div className="flex max-w-xl flex-col gap-6">
            <Field
              ...
              hint="Opcional. O cliente precisará informar para ver a galeria."
            >
              <input
                type="password"
                value={password}
            ...
            <Field
              ...
              hint="Após esta data a galeria deixa de ser acessível."
            >
